    ...
    if size:
        if item_id in list(cart.keys()):
            if size in cart[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f' You Added size{size.upper()} {product.name} Quantity to {cart[item_id][size]}')
            else:
                cart[item_id]['items_by_size'][size] = quantity
                messages.success(request, f' You Added size{size.upper()} {product.name} to your cart')
        else:
            cart[item_id] = {'items_by_size': {size: quantity}}
            messages.success(request, f' You Added size{size.upper()} {product.name} to your cart')
    else:
        if item_id in list(cart.keys()):
            ...
            messages.success(request, f' You updated {product.name} Quantity {cart[item_id]}')
        else:
            cart[item_id] = quantity
...
def remove_from_cart(request, item_id):
    ...
    try:
        ...
        messages.error(request, f'Thre was an error removing item {e}')
